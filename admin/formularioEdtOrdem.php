    ...
    <div class="container">
        ...
        <?php
            ...
            if(isset($_POST['selecionar'])) {
                $ordem = new ordensModel();
                $resp = $ordem->selecionar($_POST['idordem']);
            }
        ?>
        <div id="edtOrdem">
            <form method="post" id="edtarordem" enctype="multipart/form-data" action="controllers/ordensController.php">
                <input type="hidden" name="idordem" id="idordem" value="<?php echo $resp['idordem'];?>" >

                <div class="modal-body">

                    <div>
                        <label for="nordem">Nº Ordem</label>
                        <input type="text" name="numero_ordem" id="numero_ordem" required placeholder="Ex: OS021" value="<?php echo $resp['numero_ordem'];?>"  class="form-control">

                    </div>

                    <div>
                        <label for="data">Data Abertura</label>
                        <input type="date" name="data_abertura" required id="data_abertura"  value="<?php echo $resp['data_abertura'];?>"  class="form-control">
                    </div>
                    <?php
                    ...
                    <div>
                        ...
                        <select id="consumidores" name="consumidores" required class="buscaSelect form-control">
                            ...
                            <?php foreach ($clientes as $c){?>
                                <option value="<?php echo $c['idcliente']?>" <?php if($c['idcliente'] == $resp['idcliente']){echo "selected";}?>><?php echo $c['nome']?></option>
                            <?php }?>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="produtos" name="produtos" required class="buscaSelect form-control">
                            ...
                            <?php foreach ($produtos as $p){?>
                                <option value="<?php echo $p['idproduto']?>" <?php if($p['idproduto'] == $resp['idproduto']){echo "selected";}?>><?php echo $p['descricao']?></option>
                            <?php }?>
                        </select>
                    </div>
